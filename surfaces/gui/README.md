...
Same codebase runs in a browser (dev) and as the TonWorker desktop app.
...
1. **Start the server** (needs a model key, e.g. `OPENAI_API_KEY`, in the environment —
   ...
   ```bash
   ./.venv/bin/tonworker-server --cwd /path/to/your/project --port 8765
   ```
2. **Start the UI:**
   ```bash
   cd surfaces/gui
...
in dev it finds the server at `.venv/bin/tonworker-server` automatically (a
