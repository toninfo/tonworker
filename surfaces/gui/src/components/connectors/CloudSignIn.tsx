...
export function CloudSignInInline({ blurb }: { blurb?: string }) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      >
        {waiting ? t("Check your browser…") : t("Sign in to TonWorker Cloud")}
      </button>
      <div className="text-[11.5px] text-faint">
        {blurb ||
          t("Sign-in unlocks one-click connects — or switch to Manual, which works without it.")}
      </div>
    </div>
  );
}
...
export function CloudStatusPending() {
  ...
  return (
    ...
    >
      {t("Checking TonWorker Cloud sign-in…")}
    </div>
  );
}
