...
function relayHealth(slack: SlackStatus | null): { dot: string; text: string } {
  if (!slack) return { dot: "bg-ok", text: translate("Live · managed relay") };
  if (!slack.signed_in)
    return { dot: "bg-warnInk", text: translate("Sign-in needed — relaying is paused") };
  if (slack.relay.state === "offline")
    return { dot: "bg-faint/60", text: translate("Offline — can't reach the relay") };
  if (slack.relay.state === "reconnecting")
    return { dot: "bg-warnInk", text: translate("Reconnecting to the relay…") };
  return { dot: "bg-ok", text: translate("Live · managed relay") };
}
    ...
    <div data-testid="slack-workspaces">
      ...
      {!c.connected && (
        <div className={GRP}>
          <div className={ROW + " text-[12.5px] text-muted"}>
            ...
            {cloud?.signed_in
              ? ""
              : ` ${t("One-click needs cloud sign-in; Manual works without it.")}`}
          </div>
        </div>
      )}
      ...
      {c.connected && !relay && (
        <div data-testid="slack-manual-card">
          <div className={GRP_H}>
            {c.account || t("workspace")}{" "}
            <span className="font-normal text-faint">· {t("manual tokens")}</span>
          </div>
          <div className={GRP}>
            <PeopleRow
      ...
      {c.connected && (
        <div className={FOOT + " mt-2"}>
          {t("Names come from Slack automatically. IDs show on hover.")}
        </div>
      )}
...
function PersonPicker({
  ...
  const { t } = useI18n();
  const resolvedLabel = buttonLabel ?? t("＋ Add person");
      ...
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        ...
        {!editable && owners.length > 0 && (
          <span className="text-[11.5px] text-faint">
            {t("Set by the workspace installer.")}
          </span>
        )}
    ...
    <div className={ROW + " bg-warnSoft/25"} data-testid={`waiting-${m.id}`}>
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="text-[12.5px] text-muted">
          {t("in {place}", { place: m.chat_name || m.chat_id })}
        </span>
