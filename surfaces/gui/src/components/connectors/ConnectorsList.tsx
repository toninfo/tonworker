...
// rows navigate to the connector's detail subpage. Available connectors below with
// a Connect pill that opens the manual-setup modal (no relay / cloud chips).
          ...
          <div className={GRP}>
            {connected.map((c) => (
              ...
              >
                ...
                <span className={CHIP_OK}>● {c.two_way ? t("Live") : t("Ready")}</span>
...
function statusLine(c: Connector): string {
  if (c.name === "slack" && (c.workspaces?.length ?? 0) > 0) {
    const n = c.workspaces!.length;
    return n === 1 ? t("{n} workspace", { n }) : t("{n} workspaces", { n });
  }
