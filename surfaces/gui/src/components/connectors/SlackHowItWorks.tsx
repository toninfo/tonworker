...
// reads as a screenshot of Slack), TonWorker on the right (app tokens). Tabs
...
const TAB_KEYS = [
  "Mention → session",
  "Threads stay connected",
  "Allow teammates",
] as const;
const CAPTION_KEYS = [
  "Mention @TonWorker in any channel it's invited to — a session opens here, and the answer lands back in Slack as a thread.",
...
export function SlackHowItWorks({ workspaces }: { workspaces: SlackWorkspace[] }) {
  ...
  const tourRef = useRef<number>(TAB_KEYS.length); // auto-advances left in the one-time story tour
  ...
  return (
    ...
    <div className="mb-5" data-testid="slack-howitworks">
      ...
      <div className="text-[12px] text-muted mt-0.5">
        ...
        {t("{name} connected", { name: ws?.account || t("Workspace") })}
        {mine
...
function SlackRail({ active }: { active: string }) {
  ...
  return (
    <div className="hiw-slrail">
      ...
      <div className="hiw-slnav"><span className="hiw-appav">OW</span>TonWorker</div>
    </div>
  );
}
  ...
  return (
    <div className="hiw-win hiw-ow">
      <div className="hiw-owtop">
        <span className="hiw-dots"><i /><i /><i /></span> TonWorker
      </div>
...
function OwRail({ hot, hotSub, glow }: { hot?: string; hotSub?: string; glow?: boolean }) {
  ...
  return (
    <div className="hiw-owrail">
      <div className="hiw-brand">TonWorker</div>
...
function SceneMention({ meFirst, meInitial }: { meFirst: string; meInitial: string }) {
  ...
  return (
    <>
      ...
      <Sticky d="3.1s" pos={{ left: "51%", top: "8%" }}>
        {t("a @mention starts a NEW session →")}
      </Sticky>
      <Sticky d="5.8s" r pos={{ left: "27%", bottom: "5%" }}>
        {t("the answer comes back as a thread ↑")}
      </Sticky>
      <SlackWin>
        ...
        <div className="hiw-slmain">
          ...
          <div className="hiw-slmsgs">
            ...
            >
              <span className="hiw-men">@TonWorker</span> {t("summarize this thread")}
            </Msg>
          </div>
          ...
          <div className="hiw-slthread hiw-k" style={d("5.1s")}>
            ...
            <div className="hiw-tmsgs">
              ...
              <Msg av="OW" avBg="#4a154b" name="TonWorker" app ts="6:34 PM">
      ...
      <OwWin>
        ...
        <div className="hiw-owmain">
          ...
          <div className="hiw-owchat">
            <div className="hiw-bub user hiw-k" style={d("2.8s")}>@TonWorker {t("summarize this thread")}</div>
            <div className="hiw-bub agent hiw-k" style={d("3.6s")}>
              {t("Reading the thread… signups up 3.4×, top referrer is the press page.")}{" "}
              <i>({t("replying in the Slack thread")})</i>
            </div>
          </div>
          <div className="hiw-owcomposer">{t("Message TonWorker…")}</div>
        </div>
      </OwWin>
    </>
  );
...
function SceneThread({ meFirst, meInitial }: { meFirst: string; meInitial: string }) {
  ...
  return (
    <>
      <span className="hiw-spark" style={d("1.9s")} />
      <Sticky d="3.2s" r pos={{ left: "52%", top: "10%" }}>
        {t("chatting in the thread continues the SAME conversation →")}
      </Sticky>
      <SlackWin>
        <SlackRail active="launch-room" />
        <div className="hiw-slmain">
          ...
          <div className="hiw-slthread">
            ...
            <div className="hiw-tmsgs">
              ...
              <Msg av="P" avBg="#7c6cd0" name="Priya N" ts="6:36 PM" delay=".8s">
                <span className="hiw-men">@TonWorker</span> {t("break it down by country?")}
              </Msg>
              <Msg av="OW" avBg="#4a154b" name="TonWorker" app ts="6:36 PM" delay="4.8s">
      ...
      <OwWin>
        ...
        <div className="hiw-owmain">
          <div className="hiw-owtitle">
            {t("Summarize #launch-room")}{" "}
            <span className="hiw-via">{t("via Slack — same session")}</span>
          </div>
          <div className="hiw-owchat">
            <div className="hiw-bub agent hiw-stay">
              {t("…signups up 3.4×, top referrer is the press page.")}
            </div>
...
function SceneTeammates() {
  ...
  return (
    <>
      <span className="hiw-spark" style={d("1.9s")} />
      <Sticky d="3.4s" pos={{ left: "53%", bottom: "10%" }}>
        {t("first-time senders wait for your OK")}
      </Sticky>
      <SlackWin>
        <SlackRail active="launch-room" />
        <div className="hiw-slmain">
          ...
          <div className="hiw-slmsgs">
            ...
            >
              <span className="hiw-men">@TonWorker</span> {t("pull the signup numbers?")}
            </Msg>
          </div>
          <SlackComposer placeholder={t("Message #launch-room")} />
        </div>
