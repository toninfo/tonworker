      ...
      {c.mcp && !manualOnly && (
        ...
        <div className="space-y-2" data-testid="mcp-connect">
          <button className={BTN_ACCENT} onClick={mcpOneClick} disabled={waiting}>
            {waiting ? t("Check your browser…") : t("Connect {name} with one click", { name: c.title })}
          </button>
          {c.fields.length > 0 && (
          ...
          {c.managed_paused ? (
            ...
            <>
              <button className={BTN_ACCENT + " opacity-50"} disabled data-testid="managed-coming-soon">
                {t("Connect {name} with one click", { name: c.title })}
            ...
            <CloudSignInInline
              blurb={t("Sign-in unlocks the one-click {name} connect — or connect manually below.", { name: c.title })}
            />
          ) : (
