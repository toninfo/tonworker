...
function TurnGroup({
  ...
  const stepsLabel = tn("{n} step", "{n} steps", nSteps);
  ...
  return (
    <details className="stepgroup" open={open}>
      ...
      >
        ...
        <span>
          <span>{running ? t("Running {label}…", { label: stepsLabel }) : stepsLabel}</span>
          {declined > 0 && (
            <>
              {" · "}
...
// fork：保留 i18n；上游 Memory V1：支持撤回刚写入的记忆
              ...
              >
                {item.undone ? (
                  <span data-testid="memory-notice-undone">
                    {item.previous
                      ? t("Okay — put back the way it was.")
                      : t("Okay — forgotten.")}
                  </span>
                ) : (
                  <>
                    <span className="min-w-0">
                      <span className="font-medium">
                        {item.previous
                          ? t("I've updated what I remember")
