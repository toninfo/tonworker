    ...
    <Shell>
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <PanelHead
            title={t("Automations")}
            sub={t("Recurring tasks TonWorker runs on a schedule.")}
          />
        </div>
        <button
      ...
      <div className="text-[12px] text-faint flex gap-1.5 mb-4">
        ...
        <span>
          {t(
            "Runs only while tonworker-server is up — a missed schedule catches up once when it next starts.",
          )}
        </span>
      </div>
      ...
      {empty ? (
        !showForm && (
          <div className={CARD + " p-4 text-[12.5px] text-muted"}>
            {t(
              "No scheduled tasks yet — use a template above, click {action}, or just ask TonWorker in a session.",
              { action: t("+ New automation") },
            )}
          </div>
        )
      ) : (
          ...
          {tasks.map((task) => (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 text-[12px] text-muted">
                ...
                {task.enabled ? task.schedule : t("Paused")} ·{" "}
                {t("next {when}", { when: fmt(task.next_run) })} ·{" "}
                {tn("{n} run", "{n} runs", task.run_count)}
...
function TaskDetail({
  ...
  const startEdit = () => {
    ...
    const { time: cronTime, freq: cronFreq } = fromCron(task.schedule_raw?.cron);
    setTime(cronTime);
    setFreq(cronFreq);
    setEditing(true);
  };
      ...
      <div className="sched-detail">
        ...
        {editing ? (
          ...
          <div className="conn-meta">
            ...
            {task.enabled
              ? t("Active · next {when}", { when: fmt(task.next_run) })
              : t("Paused")}{" "}
            · {task.schedule}
          </div>
        )}
