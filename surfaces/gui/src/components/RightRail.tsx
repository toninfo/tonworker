    ...
    <aside className={"right-rail" + (selected ? " artifact-mode" : "")}>
      {selected ? (
        ...
        <>
          ...
          {showArtifacts && (
          <RailSection
            title={artifacts.length ? t("Artifacts ({n})", { n: artifacts.length }) : t("Artifacts")}
            open={open.artifacts}
...
function ProgressSummary({ running, toolNames, todo }: { running: boolean; toolNames: string[]; todo: TodoItem[] }) {
      ...
      <div className="rail-todo-list">
        ...
        {running && (
          <div className="rail-muted">
            {toolNames.length ? tn("{n} tool call so far.", "{n} tool calls so far.", toolNames.length) : t("Working...")}
          </div>
        )}
      </div>
    );
  ...
  if (running) {
    return (
      <div className="rail-muted">
        {toolNames.length ? t("Working on this task with {n} tool calls so far.", { n: toolNames.length }) : t("Working on this task.")}
      </div>
    );
  }
  return (
    <div className="rail-muted">
      {t("For longer multi-step tasks, progress will appear here while TonWorker plans, uses tools, waits for approval, and produces artifacts.")}
    </div>
  );
}
      ...
      <div className="artifact-preview">
        {!content ? (
          ...
          <div className="artifact-open-prompt">
            ...
            <p>{t("This {kind} file can’t be previewed here.", { kind: /\.pptx?$/i.test(artifact.name) ? t("PowerPoint") : t("Word") })}</p>
      ...
      {(note || body.length > MAX_TABLE_ROWS) && (
        <div className="rail-muted artifact-table-note">
          ...
          {body.length > MAX_TABLE_ROWS
            ? t(" Showing first {shown} of {total} rows.", {
