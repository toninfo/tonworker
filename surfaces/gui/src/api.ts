...
const fetch = (
  ...
  if (token) headers.set("X-TonWorker-Token", token);
...
const openWebSocket = (url: string): WebSocket => {
  const token = apiToken();
  return token
    ? new WebSocket(url, ["tonworker", token])
    : new WebSocket(url);
};
...
// --- TonWorker Cloud (optional sign-in; manual token paste always works) ---
...
// bot is a member (Slack API constraint); is_member=false → "invite @TonWorker" hint.
