//! TonWorker desktop shell.
//!
//! Tauri is a thin native window over the existing React SPA. It:
//!   1. picks a free localhost port and starts the Python `tonworker-server` as a managed
...
fn server_bin() -> PathBuf {
    ...
    if cfg!(windows) {
        ...
        p.push("../../../.venv/bin/tonworker-server");
    }
    p
}
...
/// The sidecar's log file: `<state_dir>/logs/tonworker-server.log`, fresh per
...
fn server_log_file() -> Option<std::fs::File> {
    ...
    let path = dir.join("tonworker-server.log");
    if path.exists() {
        let _ = std::fs::rename(&path, dir.join("tonworker-server.log.old"));
    }
...
async fn install_update(
    ...
    // Exit path's sidecar kill runs via RunEvent, so no orphaned tonworker-server.
    app.restart();
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let open_i = MenuItem::with_id(app, "open", "打开 TonWorker", true, None::<&str>)?;
            ...
            // System tray icon: Ton 红标 rgba（与 src-tauri/icons/tray.rgba 同源，编译期嵌入）
        ...
        .expect("error while building the TonWorker desktop app")
        .run(|app, event| {
