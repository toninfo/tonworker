...
def test_channel_name_resolves_to_team_qualified_address(tmp_path, monkeypatch):
    ...
    out = tool("slack:#all-tonworker", "Hi")
...
def test_bare_channel_names_coerce_to_slack(tmp_path, monkeypatch):
    """The owner's exact transcript: the model sent target='all-tonworker' and
    '#all-tonworker' — no 'slack:' prefix — and got 'invalid target'. Bare names
    ...
    assert tool("all-tonworker", "Hi")["ok"] is True
    assert tool("#all-tonworker", "Hi")["ok"] is True
...
def test_unknown_ambiguous_and_not_member_names_error_actionably(tmp_path, monkeypatch):
    ...
    assert "invite @TonWorker" in tool("slack:#private-ops", "Hi")["error"]
