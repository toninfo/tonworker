...
def _browser_page(
    ...
    return (
        ...
        f"<title>{_html.escape(title)} — TonWorker</title><style>"
        ...
        "</style></head><body>"
        '<div class="card"><div class="mark"><i></i>TonWorker</div>'
        f"{icon}<h1>{_html.escape(title)}</h1><p>{_html.escape(detail)}</p>{err}</div>"
        '<div class="foot">Served locally by TonWorker on your Mac</div>'
        "</body></html>"
    )
...
_CONNECT_FAILED_DETAIL = (
    ...
    "Close this tab and try again from TonWorker."
)
...
def create_app(manager: SessionManager) -> FastAPI:
    ...
    def _request_authenticated(request: Request) -> bool:
        # Accept legacy X-OpenWorker-Token during rename; header names are case-insensitive.
        provided = (
            request.headers.get("x-tonworker-token", "")
            or request.headers.get("x-openworker-token", "")
        )
        return bool(
            api_token
            and provided
    ...
    def _websocket_brand_subprotocol(ws: WebSocket) -> str | None:
        """Echo the brand subprotocol the client offered (tonworker, or legacy openworker)."""
        if not api_token:
            return None
        protocols = {
            part.strip()
        ...
        if "tonworker" in protocols:
            return "tonworker"
        if "openworker" in protocols:
            return "openworker"
        return "tonworker"
    ...
    async def require_sidecar_token(request: Request, call_next):
        ...
        return JSONResponse(
            {"error": "missing or invalid TonWorker sidecar token"},
            status_code=401,
        )
    ...
    async def mcp_oauth_callback(
        ...
        if error:
            return HTMLResponse(
                _browser_page(
                    "Sign-in failed",
                    "The service reported an error. Return to TonWorker and try again.",
                    ok=False,
                    error=error,
                ),
                status_code=400,
        ...
        if not code or not mcp_oauth.deliver_callback(code, state or None):
            return HTMLResponse(
                _browser_page(
                    ...
                    "The sign-in may have timed out. Return to TonWorker and start it again.",
                    ok=False,
                ),
                status_code=400,
            )
        return HTMLResponse(
            _browser_page(
                "Connected",
                "Sign-in complete. You can close this tab and return to TonWorker.",
                ok=True,
            )
        )
    ...
    # -- TonWorker Cloud: sign-in + managed one-click connect ---------------
    ...
    def cloud_login() -> dict[str, Any]:
        """Start browser sign-in. Disabled in this fork (no Auth0 / cloud broker)."""
        import webbrowser
        ...
        # Stub returns ok:false with no authorize_url — never open a browser.
        url = out.get("authorize_url")
        if not out.get("ok", True) or not url:
            return {"ok": False, "error": out.get("error") or "cloud login unavailable"}
        webbrowser.open(url)
    ...
    async def cloud_auth_callback(code: str = "", state: str = "", error: str = ""):
        ...
        signin_failed_detail = (
            "Close this tab and try signing in again from TonWorker."
        )
        if error:
            return HTMLResponse(
                _browser_page(
        ...
        return HTMLResponse(
            _browser_page(
                "Signed in",
                "You're signed in to TonWorker Cloud. "
                "You can close this tab and return to TonWorker.",
            )
        )
    ...
    async def ws_session(ws: WebSocket, session_id: str) -> None:
        ...
        if not _origin_allowed(ws.headers.get("origin")):
            await ws.close(code=1008)
            return
        await ws.accept(subprotocol=_websocket_brand_subprotocol(ws))
