...
#   3. `tauri build --bundles app` → TonWorker.app (resources are copied in).
...
echo "==> [1/5] PyInstaller: bundling tonworker-server ($TRIPLE)"
...
cp -RL "$HERE/dist/tonworker-server" "$GUI/src-tauri/binaries/sidecar"
...
if [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  find "$SIDECAR" -type f ! -name "tonworker-server" \
  ...
  codesign --force --sign "$APPLE_SIGNING_IDENTITY" --timestamp --options runtime \
    --entitlements "$GUI/src-tauri/entitlements.plist" "$SIDECAR/tonworker-server"
fi
